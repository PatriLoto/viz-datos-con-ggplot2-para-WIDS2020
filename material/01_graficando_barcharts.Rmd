...
Para poder observar qué datos contiene, utilizaremos la función `View`


```{r datos_con_DT, echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot2 incluye muchas funciones geom, en este caso, elegimos la función **geom_bar()**, la cual tiene varios argumentos como `mapping = NULL,`
`data = NULL`, `stat = "count"`, `position = "stack"` entre otros. Nosotros, utilizaremos la opción `stat="identity"`, la cual nos permite relacionar la altura de las barras con los valores brutos de la variable 'y', y la opción `position = "stack"`, con la cual le decimos a ggplot que utilizaremos barras del tipo apiladas.
...
Si no deseas un gráfico de barras apiladas podes usar una de las otras tres opciones, `position= identity`(identidad), `position= dodge` (contiguas) o `position= fill`(llenar).
...
```{r position_stack, echo=TRUE, fig.align='center', fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
ggplot(data=subset_propina, aes(x=time, y=total_bill, fill=sex))+
