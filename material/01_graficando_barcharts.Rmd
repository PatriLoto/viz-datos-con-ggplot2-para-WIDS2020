...
```{r subset_ejemplo, message=FALSE, warning=FALSE, include=FALSE}
...
```{r estructura_dataset, message=FALSE, warning=FALSE}
str(subset_propina)
```



Para poder observar qué datos contiene, utilizaremos la función `print`


```{r datos_con_DT, message=FALSE, warning=FALSE}

print(subset_propina)
...
* Capas(layers): al menos una capa que describe cómo representar cada observación, usualmente creada con una **función geom**. Además cada capa puede tener una transformación estadística stat una posición y opcionalmente un conjunto de mapeos estéticos
...
```{r datos, echo=TRUE,message=FALSE, warning=FALSE, fig.height=6, fig.width=8, fig.align='center', fig.show = "hold"}
...
```{r atributos_esteticos, echo=TRUE,message=FALSE, warning=FALSE, fig.height=6, fig.width=8, fig.align='center', fig.show = "hold"}
...
ggplot2 incluye muchas funciones geom, en este caso, elegimos la función **geom_bar()**, la cual tiene varios argumentos como `mapping`
`data`, `stat`, `position` aes como `width` y `color`, entre otros. Nosotros, utilizaremos la opción `stat="identity"`, la cual nos permite relacionar la altura de las barras con los valores brutos de la variable 'y', y la opción `position = "stack"`, con la cual le decimos a ggplot que utilizaremos barras del tipo apiladas.
...
Si no deseas un gráfico de barras apiladas puedes usar alguna de las otras tres opciones, `position= identity`(identidad), `position= dodge` (contiguas) o `position= fill`(llenar).


**"identity"** (identidad), **“dodge”** (contiguas), o **"fill"** (llenar).
...
ggplot(data=subset_propina,aes(x=time, y=total_bill, fill=sex))+
  geom_bar(stat="identity", position= position_dodge(),  width=.7, color="black")
```




  - b. Gráfico de barras llenas con la opción **position_fill**



```{r position_fill, echo=TRUE, fig.align='center', fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
ggplot(data=subset_propina, aes(x=time, y=total_bill, fill=sex))+
  geom_bar(stat = "identity", position=position_fill(), width=.7, color="black")
...
```{r coord_escalas_4, echo=TRUE, fig.align='center', fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
ggplot(data=subset_propina, aes(x=time, y=total_bill, fill=sex))+
  geom_bar(stat = "identity", position=position_dodge(), width=.7, color="black")+
  scale_x_discrete(limit=c("Lunch","Dinner"), labels=c("Almuerzo", "Cena"))+
...
```{r theme_bw, echo=TRUE, fig.align='center', fig.height=8, fig.width=6, message=FALSE, warning=FALSE, highlight=TRUE, tidy=TRUE}
...
```{r grafico_ggtext ,echo=TRUE, fig.align='center', fig.height=8, fig.width=6, message=FALSE, warning=FALSE, highlight=TRUE, tidy=TRUE}
