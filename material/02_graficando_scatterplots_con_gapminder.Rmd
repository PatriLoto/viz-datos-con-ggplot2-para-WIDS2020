...
Antes de empezar a graficar, inspeccionaremos el dataset `gapminder`, el cual es un paquete que contiene datos sobre población, expectativa de vida y PIB per cápita de los cinco continentes y sus 
respectivos países entre los años 1952 y 2007. 

En primer lugar, utilizaremos la función `str` para conocer la estructura de nuestro conjunto de datos, es decir, cuántas observaciones contiene, cuáles variables y de qué tipo son, tal como se 
observa:
...
los datos, e incluso buscar algún dato en particular, por ejemplo, los datos correspondientes a Argentina.
...
Luego de darle un vistazo a los datos, estamos en condiciones de comenzar a graficar, el primer paso es `ggplot(data = gapminder)`:


```{r datos_1, echo=FALSE,warning=FALSE, fig.height=8, fig.width=10, fig.show="hold" }
...
Para graficar un **scatter plot** elegimos la función `geom_point`.
...
Utilizaremos la librería [patchwork](https://github.com/thomasp85/patchwork) para combinar dos gráficos (el gráfico **pTheme_final** con el gráfico facetado **p_facet_ipsum_tw**),pero antes debo generarlos nuevamente sin título, ni subtítulo ni caption, ya que esos elementos los agrego al unirlos con patchwork.
...
-  `p1 / p2`, la primer figura en la parte superior y la segunda abajo de esta.


```{r combino_graficos, message=FALSE, warning=FALSE, fig.height=12,  fig.width=14, fig.show="hold"}
library(patchwork)

patchwork <- p1/ p2+         #combino ambos gráficos
...
```{r para_plotly, message=FALSE, warning=FALSE,include=FALSE, fig.height=6,fig.width=8, fig.show="hold", fig.width=8}

#Volvemos a ejecutar el gráfico con asignandole un tamaño menor al título para que quedé mejor en el
# gráfico interactivo. 
...
```{r graficos_plotly,fig.show="hold", fig.height=6, fig.width=9, message=FALSE, warning=FALSE}
...
```{r graficos_plotly_2,fig.show="hold", fig.height=7,fig.width=8, message=FALSE, warning=FALSE}
ggplotly(p_facet)
