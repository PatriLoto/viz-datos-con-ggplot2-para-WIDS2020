...
Necesito volver a ejecutar el gráfico de la sección 9 para formatear la presentación de los variables, 'continente', 'ingreso per cápita' y 'esperanza de vida' que luego se mostraran en el gráfico interactivo. ¿Cómo lo hago? En la parte de los mapeos estéticos agregó: 

`text = paste('<b>Continente:</b> ' , continent,'<br />','<b>Ingreso per cápita:</b> ','$', round(gdpPercap,2),'<br>', '<b>Expectativa de vida:</b> ', round(lifeExp,1))))`
`


```{r para_plotly, eval=FALSE, fig.height=6, fig.show="hold", fig.width=8, message=FALSE, warning=FALSE, include=FALSE}

# #Volvemos a ejecutar el gráfico con asignandole un tamaño menor al título para que quedé mejor en el gráfico interactivo. 
text_size <-14   # tamaño base del texto
margin_size <- text_size/2  #de los margenes

p_capa_smooth <- ggplot(data=gapminder, mapping=aes(x= gdpPercap, y= lifeExp , text = paste('<b>Continente:</b> ' , continent,'<br />','<b>Ingreso per cápita:</b> ','$', round(gdpPercap,2),'<br>', '<b>Expectativa de vida:</b> ', round(lifeExp,1)))) +
  geom_point(aes(color=continent),size=3,alpha=0.6) +
  geom_smooth(method = 'gam', col = "#2c204d", size = 0.7,
              fill = "gray60", alpha = 0.2) +
  scale_x_log10(labels= scales::dollar) +
...
Con la sentencia `ggplotly(p_capa_smooth, tooltip = "text")` convierto la figura anterior en una interactiva, con 'tooltip = "text"' le indicó que me muestre las variables con el formato especificado anteriormente.
